...
- With the current `.env.local` placeholder `INNGEST_EVENT_KEY` / `INNGEST_SIGNING_KEY`, local
  Inngest sync fails unless Next runs in explicit dev mode with those keys blanked for the process:
  `INNGEST_DEV=1 INNGEST_EVENT_KEY= INNGEST_SIGNING_KEY= pnpm dev`, then
  `npx inngest-cli@latest dev -u http://localhost:3000/api/inngest`. A clean sync shows
  `GET /api/inngest 200` and `PUT /api/inngest 200` in the Next terminal. Do not edit `.env.local`
  just to run this.
...
**Through Phase 5 (route-aware forms / VIDEX) is merged to `main` (latest PR #27).** All drafted-document verticals on the shared `drafts`/approval foundation are live: cover letter, employer letter, CV, and Anabin justification (Phase 4C, PR #25). The route-aware forms / VIDEX foundation (Phase 5-1 through 5-4) is also merged (PRs #26, #27). The phase status table, per-phase write-ups, and PR map: `docs/context-history.md`. Run DB suites **serially** — see the `EMAXPOOLSREACHED` gotcha.
...
**Anabin justification draft (4C-1, merged to main PR #25):** `anabin_justification` is the fourth draft type. It uses the existing `drafts`/approval/Inngest path, has typed Zod content (`title`, `subject`, institution/degree recognition status, paragraphs, recommended next steps), and registers `draft_anabin_justification` before `lookup_anabin` so the single cache-control breakpoint still sits on the last tool. The draft is a reviewer-facing uncertainty memo for Anabin-unknown/not-found/ZAB paths, not a deterministic recognition verdict. The Drafts tracker now has four rows; only `approved` counts complete.

**Regenerate draft with framing (4C-2, merged to main PR #25):** `regenerate_draft` creates a new version of an existing draft with a reviewer framing instruction. It verifies the source draft is in the active case, inserts the next version for the same draft type, dispatches `draft.requested` with optional `framingInstruction`, and logs only safe metadata (`framingProvided: true`, not the free text). `generateDraftByType` threads framing into every draft prompt after hard rules. The review route shows read-only version history; stale review URLs redirect to the latest ready version when possible.

**Drafts completeness signal (4C-3, merged to main PR #25):** Draft requirements now live in `config/rules/journey.yaml` under the `drafts` phase. `requiredDraftsForRoute(verdict)` filters those requirements by route plus verdict blocker/warning conditions; `computeJourneyProgress` uses that list for the Drafts phase total/completed count. Normal recognized-degree cases require cover letter, employer letter, and CV; Anabin-unknown/H- verdicts also require `anabin_justification`. Only `approved` drafts count complete. This is a tracker signal only; Phase 6 owns the blocking package gate.

**Route-aware forms (5-1 through 5-4, merged to `main` — PR #26 = 5-1+5-2A, PR #27 = 5-3+5-4):** `src/lib/drafting/videx.ts` defines the 37 field-level VIDEX map, validates every mapped Profile/CaseFacts source path via `validateLeafPath`, and returns a pure completeness report with AcroForm values plus honest missing fields. `config/rules/consulates.yaml` now has structured `formMode`: Bengaluru is `csp_integrated`, Toronto is `videx_online`. `src/lib/forms/output.ts` exposes `requiredFormOutputForCase(caseFacts)` so UI/tools branch from config instead of parsing prose; `src/lib/forms/view-model.ts` combines that route mode with VIDEX completeness for the workspace. `fill_videx_form` is read-only and returns `{type:'videx_completeness_result',version:1,data}` with `formOutput`; `request_missing_field` is read-only and returns `{type:'missing_form_field_request',version:1,data}` for one actionable `missing_source` field, with exact source paths the model should later write via `update_case`. Both tools are registered before `lookup_anabin`, which remains the final tool and sole cache-control breakpoint. The center workspace now includes a read-only `FormsSection` below the tracker cards with route mode, readiness gauge, official consulate/form copy, source link, and missing-field groups. Form missing-field tasks are generated by `reconcileCaseTasks` only after `target.targetConsulate` is persisted in CaseFacts, at most one next actionable `missing_source` gap at a time; `not_modelled` and `manual_signature` gaps do not become applicant tasks. Bengaluru renders as CSP-integrated readiness and does not claim a generated VIDEX PDF; Toronto renders as VIDEX readiness with Canada sources still marked not user-verified. `/case/[id]` must pass the repository-loaded profile into eligibility/progress/forms; an empty profile shell makes profile-backed form fields look missing. Source lookup decision: `vitazok/formular` is empty; the useful reference is `vitazok/immigration` (`src/lib/assembly/form-mapper.ts`, confirmed AcroForm IDs in its `CLAUDE.md`). The old repo is Schengen-short-stay shaped, so Relomate ports AcroForm names/transform patterns but not product assumptions. Current Profile/CaseFacts lacks several full VIDEX leaves (passport issue date/authority, birth country, phone/email, prior visas/fingerprints, signature), so 100% form readiness remains blocked until those leaves are modelled; the report marks them missing instead of inventing placeholders. Do not build a generic Germany-wide VIDEX PDF pipeline; future PDF/export work is Toronto-specific unless the official routes change.

**Next up** (sliced into session cards in `IMPLEMENTATION_PLAN.md`):
- **5 (route-aware forms)** — field map, form mode foundation, Forms workspace UI, and conversational gap-filling are merged (PRs #26, #27); next likely slice is either modelling the remaining VIDEX leaves needed for 100% readiness or the Toronto-only VIDEX output strategy (5-2B), not a generic Germany-wide PDF pipeline.
