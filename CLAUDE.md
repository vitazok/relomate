...
- **Phase 1B split into 3 sub-phases.** Spec at `docs/superpowers/specs/2026-05-27-phase-1b-design.md`. Sub-phases:
  - **1B-1 (in progress):** persistence + `update_case`. Plan at `docs/superpowers/plans/2026-05-27-phase-1b-1-persistence.md`. Tasks 0–5 of 13 done on `main` (NOT yet pushed; push at Task 13). Working tree clean.
  - **1B-2 (not yet planned):** Auth.js v5 magic-link via Resend + anonymous→authed continuity.
  - **1B-3 (not yet planned):** AI SDK v5 streaming chat + 3-col workspace + Inngest scaffold.
- **What's in the repo (1B-1 progress):** All of Phase 1A · `ai@^5` + `tsx@^4` deps · first Drizzle migration (`drizzle/0000_bright_spyke.sql`) applied to fresh Supabase EU project (eu-north-1; `.env.local` and `.env.test.local` both point at it) · `confidence` columns on change tables fixed `integer → numeric(3,2)` · path utilities (`src/lib/case/paths.ts`: `validateLeafPath`, `validateLeafValue`, `setAtPath`, `getAtPath`, `flattenForChangeLog`) · repository types (`src/lib/case/types.ts`: `UpdateCaseInputSchema`, `ContradictionReport`, `UpdateCaseResult`) · per-file Postgres test schema infra (`tests/_db/setup.ts` + `seed.ts`; `createTestSchema()` returns a handle with a Pool whose `search_path` is embedded via libpq `options=-c search_path=<schema>`) · repository `createCase` + `loadCase` (`src/lib/case/repository.ts`).
- **Where 1B-1 stands:** Tasks 6–13 remaining: `applyUpdate` (single path, multi-path, profile-paths, contradictions, validation errors, concurrent writes), `update_case` tool adapter, smoke script, verification + push.
...
### 1B-1 carry-over for the next agent (resume at Task 6)

- **Test schema isolation:** `tests/_db/setup.ts` already strips `"public"."<table>"` references from migration SQL before applying to test schemas (drizzle-kit emits these in FK clauses; without stripping they'd cross-schema-reference back to `public`). Don't re-add them.
- **Pool search_path:** the test pool URL has `options=-c%20search_path=<schema>` baked in — every connection in the pool starts in the test schema. Repository code MUST NOT call `SET search_path = ...` itself (it would interfere with this). The `schemaName` parameter on `makeRepository(db, schemaName)` is currently unused in `createCase`/`loadCase` for that reason. Task 6's `applyUpdate` may need the schema name only for `sql.raw` interpolation in `FOR UPDATE` lock queries, if drizzle's typed builders don't expose `FOR UPDATE` cleanly.
- **Lazy `require()` in repository:** `src/lib/case/repository.ts:36-39` uses `require('@/lib/db/client').db` to avoid env-validation at test-import time. Works (build is green) but is non-idiomatic; consider switching to async dynamic import or making `db` a required parameter — flag for later cleanup, don't block on it.
- **`Profile` schema is currently strict.** Per the plan, Task 8 will relax `ProfileSchema` fields to `Optional(...)` so partial profile writes parse. `loadCase` currently treats absent profile rows as `null` and only parses if a row exists — so reading works without Task 8's softening. But once Task 6/8 starts WRITING profile-level paths, the `Profile` schema must be relaxed first or `ProfileSchema.parse(merged)` will throw on the safety-belt step.
- **CaseFacts safety belt:** the plan calls for `CaseFactsSchema.parse(merged)` before `UPDATE case_facts` in `applyUpdate`. This catches malformed merges. Keep it.
- **Activity log payload shape (one row per `applyUpdate`, not per path):** `{kind: 'case.facts.updated', paths: [...], source, sourceTurnId, contradictions: number}`. Don't drift from this — Phase 1B-3 reads it.
- **Contradiction semantics:** path-local only ("same path written twice with different values at same-or-higher confidence"). Cross-field contradictions are eligibility-engine territory. Surfaced in result, NOT blocking the write — both writes persist; the agent decides how to surface them to the user.
