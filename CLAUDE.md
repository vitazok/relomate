...
**Route-aware forms foundation (5-1 + 5-2A, current branch):** `src/lib/drafting/videx.ts` defines the 37 field-level VIDEX map, validates every mapped Profile/CaseFacts source path via `validateLeafPath`, and returns a pure completeness report with AcroForm values plus honest missing fields. `config/rules/consulates.yaml` now has structured `formMode`: Bengaluru is `csp_integrated`, Toronto is `videx_online`. `src/lib/forms/output.ts` exposes `requiredFormOutputForCase(caseFacts)` so UI/tools branch from config instead of parsing prose. `fill_videx_form` is read-only and returns `{type:'videx_completeness_result',version:1,data}` with `formOutput`; it is registered before `lookup_anabin`, which remains the final tool and sole cache-control breakpoint. Source lookup decision: `vitazok/formular` is empty; the useful reference is `vitazok/immigration` (`src/lib/assembly/form-mapper.ts`, confirmed AcroForm IDs in its `CLAUDE.md`). The old repo is Schengen-short-stay shaped, so Relomate ports AcroForm names/transform patterns but not product assumptions. Current Profile/CaseFacts lacks several full VIDEX leaves (passport issue date/authority, birth country, phone/email, prior visas/fingerprints, signature), so the report marks those missing instead of inventing placeholders. Do not build a generic Germany-wide VIDEX PDF pipeline; future PDF/export work is Toronto-specific unless the official routes change.

**Next up** (sliced into session cards in `IMPLEMENTATION_PLAN.md`):
- **5 (route-aware forms)** — field map + form mode foundation are current-branch complete; next likely slice is Forms workspace UI or Toronto-only VIDEX output strategy, not a generic Germany-wide PDF pipeline.
