...
**Everything through Phase 3A (document-ingest) is merged to `main`.** Phase 0, 1A, 1B-1/2/3, 2A.1, 2A.2, 2B (journey-tracker, PR #3), 2C layers 1+2a (PR #4), 2C-tail L2b (PR #5), codebase-review hardening (PR #7), docs reorg (PR #8), Phase 3A document-ingest (PR #9). Per-phase write-ups, specs, and the PR map: `docs/context-history.md`. ~252 tests green (run **serially** — see the `EMAXPOOLSREACHED` gotcha).

**Next up — pick one:**
- **3B — approvals & review (the natural next slice; unblocked by 3A):** `approvals` table + primitive, `confirm_extraction` tool, side-by-side review/correct UI, confirmed extracted data → `update_case` → CaseFacts/identity. This is what makes 3A's `awaiting_confirmation` documents actually flow into the case. Reused by Phase 4 drafts. (3A deliberately stops at `awaiting_confirmation` — rule 5.)
