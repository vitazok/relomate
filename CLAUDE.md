...
- **Anthropic allows max 4 `cache_control` breakpoints.** The 4 tools each carry one (`makeXTool` `providerOptions`), which is exactly the limit — do NOT add a top-level `streamText` `providerOptions.anthropic.cacheControl` (that's a 5th → 400 `max 4 blocks`). **2A.2 adds a 5th tool → re-breaches the limit; consolidate to a single tool-block breakpoint before then** (see NOTE in `agent-turn.ts`).
...
| 2A.1 agent brain | **live-smoke PASSED 2026-05-30**; not yet merged to main | plan: `docs/superpowers/plans/2026-05-29-phase-2a-1-agent-brain.md`. 128/128 tests, build/lint/tsc clean. Branch `phase-2a-1-agent-brain`. Smoke caught 3 runtime bugs (now fixed, see commits `c896824`/`791cd1e`/`41c35ba`): `ai@5`→`^6` (v3-model 500), 5th cache_control breakpoint (400), bogus `MODEL_ID` `-4-7` (404). Verified live: `update_case` self-corrects on tool errors, contradiction path, `out_of_scope` activity log + renderer output shape. |
