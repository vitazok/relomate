...
| 3A document-ingest pipeline | complete, on branch `worktree-phase-3a-document-ingest` (not yet PR'd) | `docs/superpowers/specs/2026-06-03-phase-3a-document-ingest-design.md` + plan; see "Phase 3A" subsection below |
...
### Phase 3A — document-ingest pipeline (2026-06-04, branch `worktree-phase-3a-document-ingest`)

"File in → structured data out, awaiting confirmation." A user uploads a document; it lands in Cloudflare R2; a durable Inngest workflow classifies it against the document spine and extracts structured fields with per-field confidence, parking the result on a new `documents` row in `awaiting_confirmation`. **3A does NOT touch case state** — confirmation into `CaseFacts`/identity is 3B (rule 5 holds). Built TDD, subagent-reviewed per task; `tsc`/`eslint` clean; all Phase 3A tests green (run **serially** — `EMAXPOOLSREACHED` infra limit on full-suite re-runs, not a regression; batch DB files). Full design: `docs/superpowers/specs/2026-06-03-phase-3a-document-ingest-design.md`; runbook: `docs/runbooks/r2-reducto-setup.md`.

Load-bearing facts the next agent must know:

- **New `documents` table** (`drizzle/0003_dear_grandmaster.sql`) — `{id, caseId, userId, spineItemId, detectedType, status, r2Key, fileName, contentType, byteSize, extracted(jsonb), classification(jsonb), error, timestamps}`. **It is MUTABLE** on `status`/`extracted`/`classification`/`error` — a work-in-progress record like `case_facts`, NOT append-only. Append-only rule (10) covers `messages`/`activity_log`/`*_changes` only; the audit trail comes from the `activity_log` rows the workflow writes. Status lifecycle: `pending_upload → uploaded → classifying → extracting → awaiting_confirmation` (or `failed`).
- **NEW deps:** `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner` (R2 is S3-compatible; conventional client, no Cloudflare SDK). **NEW env:** `R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET`/`R2_ENDPOINT` (all `superRefine`-prod-required); `REDUCTO_API_KEY` (NOT prod-required — vision fallback runs without it).
- **Upload = presigned direct-to-R2** (browser PUTs bytes straight to R2, bypassing Vercel's ~4.5 MB body limit). Three routes: `POST /api/documents/upload-url` (auth+ownership+type/size validate → insert `pending_upload` row → presigned PUT url), `POST /api/documents/[id]/finalize` (ownership → `headObject` confirms bytes landed [409 if not] → status `uploaded` → emit `document.uploaded`), `GET /api/documents/[id]` (render-safe projection — NEVER leaks `r2Key` or `extracted.raw`).
- **`extractDocument` Inngest workflow** (`src/lib/inngest/functions/extract-document.ts`, registered in `api/inngest/route.ts`) — triggered by `document.uploaded` `{documentId, caseId, userId}`. Checkpointed steps: `load-document` (idempotency guard: proceeds only if status `uploaded`) → `classify` → `extract` (skips when the spine item has no extraction schema → empty fields) → `store` (`setExtraction` → `awaiting_confirmation`) → `log-extracted`. Failure → `setFailed` + `case.document.extraction_failed` activity row. **PII rule:** the `case.document.extracted` activity row carries field KEYS + confidences + `sensitiveKeys` ONLY — never values (values live only in `documents.extracted`). Follow-up (non-blocking): the load-document guard is a non-atomic read-check-write; race-safe via a conditional `setStatus ... WHERE status='uploaded'` if it ever bites.
- **Extraction provider seam** (`src/lib/extraction/`) — `makeExtractionProvider()` returns Reducto-primary + vision-fallback (`withFallback`) when `REDUCTO_API_KEY` is set, else vision-only. `ExtractionProvider` interface (`classify`/`extract`); tests inject `makeFakeExtractionProvider`. Reducto request/response shape in `reducto.ts` is a **best-effort guess — reconcile against the live API** when the key is provisioned (runbook §B). **Config-driven schemas:** `config/rules/documents.yaml` items gain an optional `extraction.fields` block (passport seeded; `passportNumber` flagged `sensitive`). No block ⇒ upload-and-store only. `src/lib/extraction/schema.ts` loads them (module-cached — restart `pnpm dev` after YAML edits).
- **R2 storage** (`src/lib/storage/r2.ts`) — `StorageAdapter` interface (`presignUpload`/`presignDownload`/`getObject`/`headObject`/`deleteObject`); `makeR2StorageAdapter()` real, `makeFakeStorageAdapter()` in-memory (`__putForTest`). `headObject` returns null on NotFound, THROWS on other errors (finalize relies on this — only null → 409). Key format: `cases/<caseId>/documents/<documentId>/<sanitized-filename>`.
- **`request_document_upload` tool** (7th agent tool) — read-only; returns `{type:'document_upload_request', version:1, data:{spineItemId, label, accept}}`. Registered in `agent-turn.ts` BEFORE `lookup_anabin` (which MUST stay last — single cache_control breakpoint; the agent-turn test asserts breakpoint count==1). Documented in `prompts/agent/v0.md` (`PROMPT_VERSION` stays `v0`). **Deviation from PRD (intentional):** extraction triggers on the finalize ROUTE, not an agent `extract_document` tool — upload is a user action; the agent never awaits background work (rule 13).
- **Renderers** `document_upload_request` (renders `DocumentUpload` drop-zone) + `document_extraction_status` in the registry. `DocumentUpload.tsx` (`'use client'`) exports the pure `uploadDocument(caseId, file)` orchestration (upload-url → R2 PUT → finalize) + a polling status card; a compact uploader is mounted in `ChatPanel`. The in-bubble card renders without `caseId` and no-ops its file input — accepted for 3A (the composer uploader is the working path); 3B/3C give documents a first-class home. **`ALLOWED_UPLOAD_TYPES`/`ALLOWED_UPLOAD_ACCEPT`** in `src/lib/documents/types.ts` is the single source of truth for the mime allow-list (route Set, tool accept-string, ChatPanel all derive from it).
- **Dev exerciser:** `scripts/dev-only/extract-doc.ts <file> <caseId>` runs the real R2 + provider path end-to-end (looks up the case owner, uploads, runs the handler, dumps the row).
- **Deferred to 3B/3C/3D (NOT gaps):** `approvals` table + `confirm_extraction` tool + side-by-side review UI (3B); full Needed/Awaiting/Confirmed Documents section + drag-drop-anywhere (3C); apostille tracker + Resend "ready for review" email (3D). Also deferred: AbortController/unmount cleanup on the poll loop; a scheduled sweep for orphaned `pending_upload` rows.

> **Docs reconciliation note (integration time):** this branch is based on `origin/main` @ PR #7, so this CLAUDE.md is the pre-PR-#8 inline structure. The in-flight docs PR #8 (`docs/mark-pr7-merged`) reorganizes CLAUDE.md and adds `docs/context-history.md`. When merging 3A, fold this "Phase 3A" subsection into whichever structure wins, and add the 3A post-mortem/write-up to `docs/context-history.md` (which exists on PR #8, not here) rather than leaving it inline.
...
### Next: 3B (approvals + confirm_extraction), OR 2C layer 3 + CI, OR 2B secondary scope

Phase 2 was sliced into sessions (well below 1M tokens each). **2A.1, 2A.2, 2B (journey-tracker), 2C layers 1+2a, and 2C-tail L2b are all done and merged to `main`.** **Phase 3A (document-ingest) is complete on `worktree-phase-3a-document-ingest`, not yet PR'd** (see the Phase 3A subsection above). Remaining:

- **3B — approvals & review (the natural next slice):** `approvals` table + primitive, `confirm_extraction` tool, side-by-side review/correct UI, confirmed extracted data → `update_case` → CaseFacts/identity (the write 3A deliberately defers). This is what makes 3A's `awaiting_confirmation` documents actually flow into the case. Reused by Phase 4 drafts.
