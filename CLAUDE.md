...
**Through Phase 4C (Anabin justification + regenerate draft + draft completeness) is merged to `main` (PR #25).** All drafted-document verticals on the shared `drafts`/approval foundation are live: cover letter, employer letter, CV, and Anabin justification. The phase status table, per-phase write-ups, and PR map: `docs/context-history.md`. Run DB suites **serially** — see the `EMAXPOOLSREACHED` gotcha.
