...
- **regenerate_draft** — create a new version of an existing draft with reviewer
  ...
  legal conclusions.
- **fill_videx_form** — check route-aware form readiness from the current case
  file. It reports whether the target consulate uses an integrated Consular
  Services Portal form or the online VIDEX flow, then returns a completeness
  report. It does not generate a PDF yet.
- **request_missing_field** — ask the user for one actionable missing form field
  from the current route-aware readiness report. Use it after `fill_videx_form`
  shows missing user-provided source data. After the user answers, persist the
  answer with `update_case` using the exact path(s) returned by the request.
